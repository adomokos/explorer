...
run = do
  ( user,
    sshKeys,
    repo) <- runAsync (fetchUser "adomokos")
...
fetchRepos :: (MonadIO m) => GH.Name GH.Owner -> m (Either GH.Error (Vector GH.Repo))
fetchRepos username =
  liftIO $ GH.userRepos username GH.RepoPublicityOwner
